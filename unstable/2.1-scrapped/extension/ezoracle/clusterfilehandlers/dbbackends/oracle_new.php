...
- the locking mechanism used for cache processing is based on "select for update",
  not on inverting mtime
- after a file delete, _exists returns false, as well as getting content

@todo implement _exclusiveLock, _freeExclusiveLock
...
class eZDBFileHandlerOracleBackend
{
    ...
    function _copyInner( $srcFilePath, $dstFilePath, $fname, $metaData )
    {
        ...
        $sql  = "INSERT INTO " . self::TABLE_METADATA . " (datatype, name, name_hash, scope, filesize, mtime, expired, lob) ";
        ...
        $sql .= "SELECT datatype, '$name', '$hash', scope, filesize, mtime, expired, lob FROM " . self::TABLE_METADATA . " WHERE name_hash=:name_hash";

        $return = $this->_query( $sql, $fname, true, array(
            //':name' => $dstFilePath,
            //':hash' => md5( $dstFilePath ),
            ':name_hash' => $metaData['name_hash'] ), self::RETURN_COUNT );
...
//        $tmpFilePath = $filePath.getmypid().'tmp';
    ...
    function _storeInner( $filePath, $datatype, $scope, $fname )
    {
        ...
        {
            // create file in db
            $sql  = "INSERT INTO " . self::TABLE_METADATA . " (name, name_hash, datatype, scope, filesize, mtime, expired, lob) " .
...
//////////////////////////////////////
    ...
    /**
     ...
     @param string $fname The function name that started the query, should contain relevant arguments in the text.
     */
    function _query( $query, $fname = false, $reportError = true, $bindparams = array(), $return_type = self::RETURN_BOOL )
    {
        eZDebug::accumulatorStart( 'oracle_cluster_query', 'oracle_cluster_total', 'Oracle_cluster_queries' );
        $time = microtime( true );
    ...
    {
        $hash = md5( $filePath );
        $query = 'SELECT filesize, lob FROM ' . self::TABLE_METADATA . " WHERE name_hash=:hash";
    ...
    **/
    function _startCacheGeneration( $filePath, $generatingFilePath )
    {
        ...
        $query = 'INSERT INTO ' . self::TABLE_METADATA . " ( name, name_hash, scope, datatype, mtime, expired ) values ( :name, :nameHash, '', '', :mTime, '0' )";
        if ( !$this->_query( $query, $fname, false, array( ':name' => $filePath, ':nameHash' => $nameHash, ':mTime' => $mtime ) ) )
        {
            $errno = $this->error['code'];
            // ORA 00001 is expected, since it means duplicate key (file is being generated)
            if ( $errno != 1 )
            {
                eZDebug::writeError( "Unexpected error #$errno when trying to start cache generation on $filePath (" . $this->error['message'] . ")", __METHOD__ );
            ...
            else
            {
                ...
                $row = $this->_selectOneRow( $query, $fname, false, false, array( ':nameHash' => $nameHash ) );
                ...
                if ( $remainingGenerationTime < 0 )
                {
                    ...
                    $res = $this->_query( $updateQuery, $fname, true, array( ':mTime' => $mtime, ':nameHash' => , ':previousMTime' => ), self::RETURN_COUNT );
