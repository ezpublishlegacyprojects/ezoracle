...
class eZDBFileHandlerOracleBackend
{
    ...
    function _delete( $filePath, $insideOfTransaction = false )
    {
        ...
        ocibindbyname($statement, ':filePathHash', $filePathHash, -1);
    ...
    function _deleteByRegex( $regex )
    {
        ...
        $statement = ociparse( $this->db, $sql );
        ocibindbyname($statement, ':escapedRegex', $escapedRegex, -1);
    ...
    function _deleteByLike( $like )
    {
        ...
        $statement = ociparse( $this->db, $sql );
        ocibindbyname($statement, ':alike', $like, -1);
    ...
    function _deleteByDirList( $dirList, $commonPath, $commonSuffix )
    {
        ...
        $sql = "DELETE FROM " . TABLE_METADATA . " WHERE name like :alike" ;
        $statement = ociparse( $this->db, $sql );
        ocibindbyname($statement, ':alike', $like, 255);
...
//        $tmpFilePath = $filePath.getmypid().'tmp';
        ...
        // unfortunately there is no lob chunked read support in PHP4
        /// @todo increase memory limit here ???
    ...
    function _copy( $srcFilePath, $dstFilePath )
    {
        ...
        $statement = ociparse( $this->db, $sql );
        ocibindbyname($statement, ':id', $srcMetadata['id'], -1);
...
/*
    ...
    function _rename( $srcFilePath, $dstFilePath )
    {
        ...
        $statement = ociparse( $this->db, $sql );
        ocibindbyname($statement, ':name', $name, -1);
        ocibindbyname($statement, ':hash', $hash, -1);
        ocibindbyname($statement, ':id', $srcMetadata['id'], -1);
        if ( !ociexecute( $statement, OCI_DEFAULT ) )
        {
            $this->_error( $statement, $sql );
    ...
    function _getFileList( $skipBinaryFiles, $skipImages )
    {
        ...
        if ( $skipImages )
            $filters[] = "'image'";
        /// @todo verify if we can use ocibindbyname inside the in() predicate...
        if ( $filters )
