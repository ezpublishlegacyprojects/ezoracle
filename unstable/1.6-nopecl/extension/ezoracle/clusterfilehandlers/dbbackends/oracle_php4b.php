...
class eZDBFileHandlerOracleBackend
{
    function _connect()
    {
        ...
        $this->db = @ociplogon( $params['user'], $params['pass'], $params['dbname'] );
        if ( !$this->db )
    ...
    function _exists( $filePath )
    {
        ...
        $row = ocifetchinto( $statement, $row, OCI_NUM );
        //$count = $row[0];
        ocifreestatement( $statement );
        return $row[0];
    }
    ...
    function _copy( $srcFilePath, $dstFilePath )
    {
		...
		$sql  =. "SELECT name, name_hash, datatype, scope, filesize, mtime, lob FROM " . TABLE_METADATA . " WHERE id = " $srcMetadata['id'];
    ...
    function _getFileList( $skipBinaryFiles, $skipImages )
    {
        ...
        $filePathList = array();
        while ( $row = ocifetchinto( $statement, $row, OCI_NUM ) )
            $filePathList[] = $row[0];
    ...
    function _fetchLob( $filePath )
    {
        ...
        if ( !( $row = ocifetchinto( $statement, $row, OCI_NUM ) ) )
        {
